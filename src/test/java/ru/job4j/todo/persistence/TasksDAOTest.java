...
public class TasksDAOTest {
    ...
    public void whenAddTaskThenSuccess() throws UserAlreadyExistException {
        ...
        UserDTO testUser = userDAO.saveUser(new UserDTO("test@test", "password", "test", new Date(1L)));
        Assert.assertThat(tasksDAO.getAllTasks().size(), is(0));
        TaskDTO newTask = tasksDAO.addTask(new TaskDTO("test_description", new Date(1L), false, testUser));
        Assert.assertThat(tasksDAO.getAllTasks().size(), is(1));
        Assert.assertThat(newTask.getDescription(), is("test_description"));
        Assert.assertThat(newTask.getCreated().getTime(), is(1L));
    ...
    public void whenUpdateTaskThenSuccess() throws UserAlreadyExistException {
        TasksDAO tasksDAO = new TasksDAO();
        UserDAO userDAO = new UserDAO();
        UserDTO newUser = userDAO.saveUser(new UserDTO("test2@test", "password", "test", new Date(1L)));
        Assert.assertThat(tasksDAO.getAllTasks().size(), is(0));
        TaskDTO newTask = tasksDAO.addTask(new TaskDTO("test_description", new Date(1L), false, newUser));
        Assert.assertThat(tasksDAO.getAllTasks().size(), is(1));
        ...
        Assert.assertThat(updatedTask.getCreated().getTime(), is(1L));
    ...
    public void whenGetAllThenReturnsAll() throws UserAlreadyExistException {
        TasksDAO tasksDAO = new TasksDAO();
        UserDAO userDAO = new UserDAO();
        UserDTO newUser = userDAO.saveUser(new UserDTO("test3@test", "password", "test", new Date(1L)));
        Assert.assertThat(tasksDAO.getAllTasks().size(), is(0));
        TaskDTO taskCompleted = tasksDAO.addTask(new TaskDTO("completed task description", new Date(1L), true, newUser));
        TaskDTO taskIncompleted = tasksDAO.addTask(new TaskDTO("incompleted task description", new Date(1L), false, newUser));
        ...
        for (TaskDTO task : result) {
            if (task.getId() == taskCompleted.getId()) {
                ...
                Assert.assertThat(task.getCreated().getTime(), is(taskCompleted.getCreated().getTime()));
                ...
                Assert.assertThat(task.getCreated().getTime(), is(taskIncompleted.getCreated().getTime()));
    ...
    public void whenGetIncompleteThenReturnsUndone() throws UserAlreadyExistException {
        TasksDAO tasksDAO = new TasksDAO();
        UserDAO userDAO = new UserDAO();
        UserDTO newUser = userDAO.saveUser(new UserDTO("test4@test", "password", "test", new Date(1L)));
        Assert.assertThat(tasksDAO.getAllTasks().size(), is(0));
        TaskDTO taskCompleted = tasksDAO.addTask(new TaskDTO("test completed task description", new Date(1L), true, newUser));
        TaskDTO taskIncompleted = tasksDAO.addTask(new TaskDTO("test incompleted task description", new Date(1L), false, newUser));
        Assert.assertThat(tasksDAO.getAllTasks().size(), is(2));
        ...
        Assert.assertThat(resultTask.getCreated().getTime(), is(taskIncompleted.getCreated().getTime()));
